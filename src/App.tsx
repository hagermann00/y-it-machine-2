...
const AppContent: React.FC = () => {
    const { state, startInvestigation, createBranch, resetProject, setActiveBranch, updateActiveBook, generatePodcast, updatePodcastEpisode, runDemoMode } = useProject();
    ...
    if (state.status === 'INPUT' || state.status === 'ERROR') {
        return (
            <>
                ...
                {/* DEMO MODE BUTTON - Floating & Theatrical */}
                <div className="fixed top-4 right-4 z-50">
                    <button
                        onClick={() => runDemoMode(DEMO_TOPICS[Math.floor(Math.random() * DEMO_TOPICS.length)])}
                        className="group relative flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-purple-600 via-pink-500 to-purple-600 bg-[length:200%_100%] animate-gradient text-white font-bold rounded-xl shadow-2xl hover:shadow-purple-500/50 hover:scale-110 transition-all duration-300 border-2 border-white/20"
                    >
                        {/* Glow effect */}
                        <div className="absolute inset-0 bg-gradient-to-r from-purple-600 via-pink-500 to-purple-600 rounded-xl blur-lg opacity-50 group-hover:opacity-75 transition-opacity"></div>

                        {/* Content */}
                        <div className="relative flex items-center gap-2">
                            <Sparkles size={20} className="animate-spin" style={{ animationDuration: '3s' }} />
                            <span className="text-lg tracking-wide">DEMO</span>
                            <span className="text-xs bg-white/20 px-2 py-0.5 rounded-full">✨ No API</span>
                        </div>
                    </button>
                </div>
