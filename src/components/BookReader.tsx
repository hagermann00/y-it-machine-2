            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-white text-black p-8 md:p-12 shadow-2xl rounded-b-xl relative">
                ...
                {activePage.type === 'CHAPTER_PAGE' && activePage.chapter && (
                    <div className="max-w-2xl mx-auto animate-fadeIn min-h-[500px] flex flex-col">
                        ...
                        {activePage.subPageIndex === 0 && activePage.chapter.visuals?.reduce((acc: React.ReactNode[], v) => { if (v.type === 'HERO') acc.push(renderVisual(v, acc.length, activePage.chapterIndex!)); return acc; }, [])}
                        ...
                        {activePage.subPageIndex === (activePage.totalSubPages! - 1) &&
                            activePage.chapter.visuals?.reduce((acc: React.ReactNode[], v) => { if (v.type !== 'HERO') acc.push(renderVisual(v, acc.length + 100, activePage.chapterIndex!)); return acc; }, [])}
                    </div>
                )}
